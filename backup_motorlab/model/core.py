...
from motorlab.model import factory
...
def setup(config: dict, train_intervals: dict, is_train: bool) -> dict:
    ...
    """
    utils.setup.fix_seed(config.get("seed", 0))

    data_dict = utils.data.load_all(
    ...
    if is_train:
        in_dim, out_dim = factory.compute_dimensions(
            data_dict=data_dict,
            in_modalities=config["data"]["input_modalities"],
            out_modalities=config["data"]["output_modalities"],
            sessions=config["sessions"],
    ...
    if uid is not None:
        ...
        checkpoint_data = factory.load_checkpoint_metadata(
            uid=uid,
