    ...
    with torch.no_grad():
        for session in dataloaders:
            for x_trial, y_trial in dataloaders[session]:
                x_trial = {
                    modality_name: modality_tensor.to(DEVICE)
    ...
    while active_sessions:
        ...
        for session in active_sessions:
            try:
                ...
                x_trial = {k: v.to(DEVICE) for k, v in x_trial.items()}
                y_trial = {k: v.to(DEVICE) for k, v in y_trial.items()}
                with autocast_context:
