...
from motorlab import keypoints, metrics, utils
from motorlab.modalities import intervals, location, poses
...
def spikes(
    ...
    even_more_data: list[np.ndarray] | None = None,
):
    if data.ndim == 1:
    ...
    if optional_data is not None:
        plt.plot(optional_data, color="red")
    if even_more_data is not None:
        for d in even_more_data:
            plt.plot(d)
...
def poses3d(
    ...
    def draw_pose(pose):
        ...
        for s, e in keypoints.get_skeleton(skeleton_type):
            x = [
                pose[keypoints.to_idx(s, experiment), 0],
                pose[keypoints.to_idx(e, experiment), 0],
            ]
            y = [
                pose[keypoints.to_idx(s, experiment), 1],
                pose[keypoints.to_idx(e, experiment), 1],
            ]
            z = [
                pose[keypoints.to_idx(s, experiment), 2],
                pose[keypoints.to_idx(e, experiment), 2],
            ]
...
def room(save_path=None):
    ...
    None
    """
    tile_width = location.x_size / location.x_divisions
    tile_height = location.y_size / location.y_divisions
    ...
    tile_number = 0
    for row in range(location.y_divisions):
        for col in range(location.x_divisions):
    ...
    ax.set_xticks(np.linspace(0, location.x_size, location.x_divisions + 1))
    ax.set_yticks(np.linspace(0, location.y_size, location.y_divisions + 1))

    ax.set_xlim(0, location.x_size)
    ax.set_ylim(0, location.y_size)
    ax.set_aspect("equal")
    plt.xlabel("x")
    plt.ylabel("y")
