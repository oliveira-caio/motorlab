...
from motorlab import data, keypoints, room
...
def export_pcs(
    session_dir: pathlib.Path | str, modality: str, coordinates: str
) -> None:
    ...
    processed_training_data = data.preprocess(
        training_data,
        cfgs={"poses": {"coordinates": coordinates}},
        session_dir=session_dir,
    )

    tr_data_stacked = np.concatenate(processed_training_data[modality], axis=0)

    pca = PCA(n_components=meta["n_signals"])
    pca.fit(tr_data_stacked)
    joblib.dump(pca, modality_dir / "meta" / f"pcs_{coordinates}_all.pkl")


def export_target(session_dir: pathlib.Path | str) -> None:
...
def _load_and_filter_trials(path: pathlib.Path) -> pd.DataFrame:
...
def _assign_cue_time(trials_info: pd.DataFrame) -> pd.DataFrame:
    ...
    trials_info["cue"] = cue
    trials_info = trials_info.drop(columns=["cuestart"])
...
def _assign_tiers(trials_info: pd.DataFrame) -> pd.DataFrame:
    ...
    for condition, indices in conditions.items():
        ...
        trials_info.loc[val_indices, "tier"] = "val"
...
def export_intervals(
    ...
    Args:
        ...
        include_homing: Whether to include homing intervals in the dataset
    ...
    trials_info["start"] = trials_info["start"].astype(int)
    trials_info["end"] = trials_info["end"].astype(int)
    trials_info["duration"] = trials_info["end"] - trials_info["start"]

    trials_info.to_csv(intervals_dir / "data.csv", index=False)
